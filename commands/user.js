...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (result.username == undefined) {
            let reponseRecherche = await fetch(
                "https://api.hyakanime.fr/user/search/" + pseudo
            );
            let dataRecherche = await reponseRecherche.text();
            var resultRecherche = JSON.parse(dataRecherche);
            if (resultRecherche != "" || undefined) {
                const usernameLePlusProche = trouveLePlusProche(
                    pseudo,
                    resultRecherche,
                    "username"
                );
        ...
        while (i < episodes) {
            ...
            if (resultatProgression[i].rewatch != undefined) {
                revisionageEpisode =
          revisionageEpisode +
          resultatProgression[i].rewatch * resultatProgression[i].progression;
        ...
        const userEmbed = new EmbedBuilder()
          ...
          "https://www.hyakanime.fr/static/media/appLogo.7fac0ec4359bda8ccf0f.png",
            ...
            .addFields(
                ...
                { name: "\u200b", value: "\u200b", inline: true },
                {
                    name: "ÉPISODES REWATCH",
                    value: "" + revisionageEpisode,
                    inline: true,
                }
            )
            .setTimestamp()
...
function trouveLePlusProche(target, items, propName) {
    return items.sort(
        (a, b) =>
            a[propName].localeCompare(target, undefined, { sensitivity: "base" }) -
      b[propName].localeCompare(target, undefined, { sensitivity: "base" })
    )[0];
}
